...
object Consumer {
  ...
  val imgChannels      = props.getProperty("consumer.model.input.channels", "1").toInt
  val classLabels      = props.getProperty("consumer.model.classes").split(",").map(_.trim)
  val triggerInterval  = props.getProperty("consumer.trigger.interval", "5").toInt
  val fileGlob         = props.getProperty("consumer.file.glob", "*.{jpg,jpeg,png}")
  val debug            = props.getProperty("debug", "false").toBoolean

  val enablePrediction = props.getProperty("consumer.enable.prediction", "true").toBoolean

  val extractLabelFromPath = props.getProperty("consumer.extract.label.from.path", "true").toBoolean

  val modelBroadcast = if (enablePrediction && Files.exists(Paths.get(modelPath))) {
    val bytes = Files.readAllBytes(Paths.get(modelPath))
    Some(spark.sparkContext.broadcast(bytes))
  } else {
    if (enablePrediction) {
      println(s"[Consumer] ATTENTION: modèle introuvable à '$modelPath'. " +
    ...
    val result = modelBroadcast match {
      case Some(bcast) =>
        val predictUdf = udf { features: Seq[Float] =>
          ...
          OnnxPredictor.predict(features.toArray, imgHeight, imgWidth, imgChannels, classLabels)
        }
        ...
        withPrediction.select(
          $"path", $"modificationTime", $"length", $"split", $"true_label",
